...
public enum EmvcoEvent implements Parcelable {
   EMVCO_STATE_START_CONFIG,
   EMVCO_STATE_START_IN_PROGRESS,
   EMVCO_STATE_START_SUCCESS,
   EMVCO_STATE_ACTIVATED,
   EMVCO_STATE_STOP_CONFIG,
   EMVCO_STATE_STOP_IN_PROGRESS,
   EMVCO_STATE_STOP_SUCCESS,
   EMVCO_STATE_STOPPED,
   EMVCO_STATE_UNKNOWN;
    ...
    public static EmvcoEvent intToEnum(int rx_event) {
        for(EmvcoEvent event:EmvcoEvent.values()) {
            if(event.ordinal() == rx_event) {
                return event;
            }
        }
        return EMVCO_STATE_UNKNOWN;
    };
}
